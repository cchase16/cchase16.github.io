...
File name: `data/PowerUps.xml`

Root:

```xml
<PowerUps version="1.0">
  ...
</PowerUps>
```
...
```xml
<PowerUpType id="string">
  <Name>string</Name>
  <Effect>pass-through-obstacles</Effect>
  <NoteColor>#RRGGBB</NoteColor>
  <GlowColor>#RRGGBB</GlowColor>
  <SpawnBaseSeconds>decimal</SpawnBaseSeconds>
  <SpawnRandomMinSeconds>decimal</SpawnRandomMinSeconds>
  <SpawnRandomMaxSeconds>decimal</SpawnRandomMaxSeconds>
  <DriftSpeed>decimal</DriftSpeed>
</PowerUpType>
```
...
- `id` is required and unique.
- `Name` is required.
- `Effect` is required. In v1 the only allowed value is `pass-through-obstacles`.
- `NoteColor` is required and must be a 7-character hex color in `#RRGGBB` form.
- `GlowColor` is required and must be a 7-character hex color in `#RRGGBB` form.
- `SpawnBaseSeconds` is required and must be a decimal greater than `0`.
- `SpawnRandomMinSeconds` is required and must be a decimal greater than `0`.
- `SpawnRandomMaxSeconds` is required and must be a decimal greater than or equal to `SpawnRandomMinSeconds`.
- `DriftSpeed` is required and must be a decimal greater than `0`.

Runtime rules:

- Power-ups drift down from the top of the field as music notes.
- Catching a power-up with the launcher activates its effect immediately.
- In v1, the active effect lasts until the next time the ball touches the bottom of the field.
- `GlowColor` is used as the active ball glow for that power-up type.
