...
export function parsePowerUpsDocument(documentNode) {
  const root = requiredRoot(documentNode, "PowerUps");
  const powerUpTypes = [...root.querySelectorAll("PowerUpType")].map((typeElement) => {
    const effect = requiredText(typeElement, "Effect");
    if (effect !== "pass-through-obstacles") {
      throw new Error(`Unsupported power-up effect "${effect}"`);
    }

    const spawnBaseSeconds = decimal(requiredText(typeElement, "SpawnBaseSeconds"), "SpawnBaseSeconds");
    const spawnRandomMinSeconds = decimal(
      requiredText(typeElement, "SpawnRandomMinSeconds"),
      "SpawnRandomMinSeconds"
    );
    const spawnRandomMaxSeconds = decimal(
      requiredText(typeElement, "SpawnRandomMaxSeconds"),
      "SpawnRandomMaxSeconds"
    );

    if (spawnRandomMaxSeconds < spawnRandomMinSeconds) {
      throw new Error("SpawnRandomMaxSeconds must be greater than or equal to SpawnRandomMinSeconds");
    }
    ...
    return {
      ...
      name: requiredText(typeElement, "Name"),
      effect,
      noteColor: hexColor(requiredText(typeElement, "NoteColor"), "NoteColor"),
      glowColor: hexColor(requiredText(typeElement, "GlowColor"), "GlowColor"),
      spawnBaseSeconds,
      spawnRandomMinSeconds,
      spawnRandomMaxSeconds,
      driftSpeed: decimal(requiredText(typeElement, "DriftSpeed"), "DriftSpeed")
    };
  });

  return {
    powerUpTypes,
    powerUpTypeById: uniqueIdMap(powerUpTypes, "power-up type")
  };
}
...
export function validateCatalog(catalog) {
  ...
  for (const powerUpType of catalog.powerUpTypes || []) {
    if (powerUpType.spawnRandomMinSeconds < 0 || powerUpType.spawnRandomMaxSeconds < 0) {
      throw new Error(`Power-up "${powerUpType.id}" has an invalid spawn window`);
    }
  }
}
