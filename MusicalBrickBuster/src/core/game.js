...
export class GameApp {
  ...
  isPointerControlZone(point) {
    return !!this.state.fieldMetrics && point.y >= this.state.fieldMetrics.field.bottom - 170;
  }
  ...
  handleCanvasPointerDown(event) {
    ...
    this.touchSession = {
      ...
      dragging: this.isPointerControlZone(point),
      moved: false,
      startX: point.x,
      startY: point.y
    };
    ...
    if (this.touchSession.dragging && !["loading", "error", "paused"].includes(this.state.mode)) {
  ...
  updateBall(deltaSeconds) {
    ...
    if (ball.y - ball.radius > field.bottom + 12) {
      ...
      if (this.state.ballsRemaining <= 0) {
        ...
        this.setStatus(
          lost > 0
            ? `Lost ${lost} streak points. ${this.isTouchLayout ? "Tap to relaunch or use Restart." : "Press Space or Restart to try again."}`
            : this.isTouchLayout ? "No balls left. Tap to relaunch or use Restart." : "No balls left. Press Space or Restart to try again."
        );
      } else {
        this.setStatus(
          lost > 0
            ? `Lost ${lost} streak points. ${this.getRetryPrompt()}`
